...
export class OrderService {
  ...
  async transition(orderId: number, to: OrderStatus, operator: string) {
    ...
    await this.prisma.orderStatusLog.create({
      data: { orderId, fromStatus: order.status, toStatus: to, operator },
    });
    return order;
  }

  async requestRefund(userId: number, id: number) {
    const order = await this.prisma.order.findFirst({ where: { id, userId } });
    if (!order) throw BizError.notFound();
    assertOrderTransition(order.status, OrderStatus.REFUNDING);
    await this.transition(id, OrderStatus.REFUNDING, 'user');
    return { ok: true };
  }
  ...
  async processRefund(orderId: number, approve: boolean) {
    const order = await this.prisma.order.findUnique({ where: { id: orderId } });
    ...
    if (!approve) {
      ...
      await this.transition(orderId, rollback, 'admin');
      return { ok: true, status: rollback };
    }

    await this.transition(orderId, OrderStatus.REFUNDED, 'admin');
